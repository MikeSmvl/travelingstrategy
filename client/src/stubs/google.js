...
_.extend = function (obj) {
	_.each(arguments, (extendObj) => {
		for (const key in extendObj) obj[key] = extendObj[key];
	});
	return obj;
};
...
_.each = function (collection, iterator) {
	if (Array.isArray(collection)) {
		for (let i = 0; i < collection.length; i++) iterator(collection[i], i, collection);
...
(function () {
	...
	getMarker = function () {
		...
		return (Marker = (function (superClass) {
			...
			extend(
				Marker.prototype,
				PositionObject.prototype,
				DraggableObject.prototype,
				VisibleObject.prototype
			);
			...
			function Marker(opts) {
				...
				if (opts != null) {
					['draggable', 'editable', 'map', 'visible', 'position'].forEach(
						(function (_this) {
							return function (o) {
						...
						return function (p) {
							return (_this[`set${capitalize(p)}`] = function (val) {
								...
								if (p === 'radius' || p === 'center') {
									return window.google.maps.event.fireAllListeners(
										`${p}_changed`,
										_this
									);
								}
		...
		getMarkerWithLabel() {
			...
			return (MarkerWithLabel = (function (superClass) {
				...
				function MarkerWithLabel(opts) {
					...
					if (opts != null) {
						['draggable', 'editable', 'map', 'path', 'visible'].forEach(
							(function (_this) {
								return function (o) {
									return (_this[o] = opts[o]);
								};
		...
		GoogleApiMock.prototype.mockEvent = function (event) {
			...
			if (!event.fireListener) {
				event.fireListener = function (thing, eventName) {
					...
					if (
						found != null
						&& (found != null ? found.events[eventName] : void 0) != null
					) {
						return found.events[eventName].forEach((cb) => {
							return cb(found.obj);
						});
					}
				};
			...
			if (!event.fireAllListeners) {
				event.fireAllListeners = function (eventName, state) {
					return listeners.forEach((obj) => {
						if (obj.events[eventName] != null) {
							return obj.events[eventName].forEach((cb) => {
								return cb(state);
							});
						}
					});
