  ...
  type: graphql.GraphQLList(Country),
    ...
    resolve: function (source, args) {
      return new Promise((resolve, reject) => {
          ...
          logger.info("Trying to query ' "+queryString+"'")
          db.run(queryString, function(err, rows) {
              if(err){
                  logger.error(err);
                  reject(err);
              }
              logger.info("'DELETE FROM "+args.origin+"(country_iso,name,advisory_text, visa_info) VALUES("+args.input.country_iso+","+args.input.name+","+args.input.advisory_text+","+args.input.visa_info+");' successfully queried")
              resolve(rows);
          });
      });
  }
